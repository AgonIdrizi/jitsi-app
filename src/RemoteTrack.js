...
class RemoteTrack extends React.Component {
  ...
  componentDidMount() {
    ...
    this.tracks = _.filter(window.sherpany.remoteTracks, rt => {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (currentTrackIdText !== previousTrackIdText) {
      ...
      this.tracks = _.filter(window.sherpany.remoteTracks, {
        ...
        switch (track.type) {
          case "audio":
            if (this.micRef.current) {
              track.track.removeEventListener(
                window.JitsiMeetJS.events.track.TRACK_MUTE_CHANGED,
                this.onTrackMuteChangedEvent
              );
    ...
    return (
      <div className="remote_track">
        ...
        <div className="remote_track_body">
          <video
            style={this.state.isVideoMuted ? { display: "none" } : {}}
            autoPlay="1"
            ref={this.videoRef}
          />
        </div>
