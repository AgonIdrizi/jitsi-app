...
class RemoteTrack extends React.Component {
    constructor (props) {
        super(props)
        this.state = {
            selectedVideoId: '',
    ...
    componentDidMount () {
        ...
        this.tracks = _.filter(window.sherpany.remoteTracks, (rt) => { return _.indexOf(trackIds, rt.id) !== -1 })
        ...
        if (videoTrack || micTrack) {
            ...
            if (micTrack) {
                ...
                this.setState({isMicMuted: micTrack.track.muted})
    ...
    componentDidUpdate (prevProps) {
        ...
        if (currentTrackIdText !== previousTrackIdText) {
            ...
            this.tracks = _.filter(window.sherpany.remoteTracks, { participantId: participantId })
            let videoTrack = _.find(this.tracks, { type: 'video' })
            let micTrack = _.find(this.tracks, { type: 'audio' })
            let newState = {}
            if (videoTrack) {
            ...
            if (track) {
                ...
                switch (track.type) {
                    ...
                    if (this.micRef.current) {
                        ...
                        track.track.removeEventListener(window.JitsiMeetJS.events.track.TRACK_MUTE_CHANGED, this.onTrackMuteChangedEvent )
                ...
                switch (track.type) {
                    case 'audio':
                        if (this.micRef.current) {
                            ...
                            track.track.addEventListener(window.JitsiMeetJS.events.track.TRACK_MUTE_CHANGED,this.onTrackMuteChangedEvent)
    ...
    render () {
        console.log('RemoteTrack renders' )
        return <div className='remote_track'>
            <div className='remote_track_controls'>
                ...
                <div className='remote_track_body'>
                    <video style={this.state.isVideoMuted ? {display: 'none'}: {}} autoPlay='1' ref={this.videoRef}/>
                </div>
            
            <div>
                <audio autoPlay='1' ref={this.micRef} />
            </div>
            {this.state.isMicMuted ? 'muted': 'unmuted' }
        </div>
