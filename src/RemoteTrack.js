...
const RemoteTrack = ({ trackIds, selectedSpeakerDeviceId }) => {
  ...
  return (
    <div>
      ...
      <div className="remote_track_body">
        <video
          style={isVideoMuted ? { display: "none" } : {}}
          autoPlay="1"
          ref={videoRef}
        />
        {isVideoMuted && <div><h4>Video Muted</h4></div>}
      </div>

      <div>
        <audio autoPlay="1" ref={micRef} />
      </div>
      {isMicMuted ? "sound muted" : " sound unmuted"}
    </div>
  );
};
